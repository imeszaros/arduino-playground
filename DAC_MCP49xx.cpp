...
DAC_MCP49xx::DAC_MCP49xx(DAC_MCP49xx::Model _model) : bufferVref(false), gain2x(false), spi_divider(SPI_CLOCK_DIV2), automaticallyLatchDual(true)
{
 /* 
  ...
  */
  ...
  if (&DAC_MCP49XX_SS_PORT == &PORTB) {
  	  DDRB |= 1 << DAC_MCP49XX_SS_PIN;
  } else if (&DAC_MCP49XX_SS_PORT == &PORTC) {
	  DDRC |= 1 << DAC_MCP49XX_SS_PIN;
  } else if (&DAC_MCP49XX_SS_PORT == &PORTD) {
	  DDRD |= 1 << DAC_MCP49XX_SS_PIN;
  }

  if (&DAC_MCP49XX_LDAC_PORT == &PORTB) {
  	  DDRB |= 1 << DAC_MCP49XX_LDAC_PIN;
  } else if (&DAC_MCP49XX_LDAC_PORT == &PORTC) {
	  DDRC |= 1 << DAC_MCP49XX_LDAC_PIN;
  } else if (&DAC_MCP49XX_LDAC_PORT == &PORTD) {
	  DDRD |= 1 << DAC_MCP49XX_LDAC_PIN;
  }

  DAC_MCP49XX_SS_PORT |= 1 << DAC_MCP49XX_LDAC_PIN;
  DAC_MCP49XX_LDAC_PORT |= 1 << DAC_MCP49XX_LDAC_PIN;
...
void DAC_MCP49xx::shutdown(void) {
  ...
  DAC_MCP49XX_SS_PORT &= ~(1 << DAC_MCP49XX_SS_PIN); // Clear
  ...
  DAC_MCP49XX_SS_PORT |= (1 << DAC_MCP49XX_SS_PIN);
}
